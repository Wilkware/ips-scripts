...
$LON = '13.4094';
#
# Image resourcen (background & overlay) and sqare size
$BACK = '/user/sunrun/bg.png';
$OVER = '/user/sunrun/chart.svg';
$SIZE = 600;
#
# Update Interval
$UPDATE = 15;
...
if ($_IPS['SENDER'] == "TimerEvent") {
    ...
    $info = DateSunInfo($LAT, $LON);
    //var_dump($info);
    // Build HTML
    $html = BuildHtml($info, $BACK, $OVER, $SIZE);
...
function BuildHtml($pos, $bg, $chart, $size)
{
    ...
    $html .= '<head>';
    $html .= '<style>';
    $html .= '#sunmap {background: url("' . $bg . '") no-repeat; background-size: cover; width: ' . $size . 'px; height: ' . $size . 'px;	}';
    $html .= 'canvas {background: url("' . $chart .'") no-repeat center; background-size: 100% 100%;}';
    $html .= '</style>';
    $html .= '</head>';
    $html .= '<body>';
    $html .= '<div id="sunmap"><canvas id="sunpos" width="' . $size . '" height="' . $size . '">Your browser does not support the HTML canvas tag.</canvas></div>';
    $html .= '<script>';
    ...
    if (($a_sp <= $a_ss) &&  ($a_sp >= $a_sr)) {
        $html .= 'ctx.fillStyle = "#F1C40F";';
        $html .= 'ctx.strokeStyle = "#FC9C54";';
    }
    else {
        $html .= 'ctx.fillStyle = "#57544B";';
        $html .= 'ctx.strokeStyle = "#F7f2E0";';
    }
	$html .= 'ctx.arc(x2, y2, 16, 0, 2 * Math.PI);';
	$html .= 'ctx.fill();';
	$html .= 'ctx.stroke();';
    $html .= '</script></body>';
    $html .= '</html>';
